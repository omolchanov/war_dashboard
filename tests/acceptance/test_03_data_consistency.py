...
def test_period_format_yyyy_mm_dd_all_endpoints(client, consistent_losses_df, consistent_economics_df, consistent_recruiting_df):
...
def test_prediction_merge_succeeds_with_consistent_data(client, consistent_losses_df, consistent_economics_df, consistent_recruiting_df):
    ...
    with (
        ...
        app.dependency_overrides[get_prediction_data] = lambda refresh=False: pred_results
        try:
            response = client.get("/prediction")
        finally:
            app.dependency_overrides.pop(get_prediction_data, None)
    assert response.status_code == 200
    data = response.json()
